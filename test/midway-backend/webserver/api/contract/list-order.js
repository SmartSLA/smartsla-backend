...
describe('GET /api/contracts/:id/orders', function() {
  let app, lib, helpers, ObjectId;
  let user1, user2, order, contractId;
  ...
  it('should respond 401 if not logged in', function(done) {
    helpers.api.requireLogin(app, 'get', `/api/contracts/${contractId}/orders`, done);
  });

  it('should respond 403 if user is not an administrator', function(done) {
    helpers.api.loginAsUser(app, user2.emails[0], password, helpers.callbacks.noErrorAnd(requestAsMember => {
      const req = requestAsMember(request(app).get(`/api/contracts/${contractId}/orders`));
  ...
  it('should respond 404 if there is invalid contract in the request params', function(done) {
    helpers.api.loginAsUser(app, user1.emails[0], password, helpers.callbacks.noErrorAnd(requestAsMember => {
      const req = requestAsMember(request(app).get('/api/contracts/invalid_objectid/orders'));
  ...
  it('should respond 200 with the list orders if list orders successfully', function(done) {
    helpers.api.loginAsUser(app, user1.emails[0], password, helpers.callbacks.noErrorAnd(requestAsMember => {
      const req = requestAsMember(request(app).get(`/api/contracts/${contractId}/orders`));
      const expectResult = [getObjectFromModel(order)];
  ...
  it('should respond 200 with the list contains only latest order if offset=0 and limit=1', function(done) {
    ...
    .then(createdOrder => {
      helpers.api.loginAsUser(app, user1.emails[0], password, helpers.callbacks.noErrorAnd(requestAsMember => {
        const req = requestAsMember(request(app).get(`/api/contracts/${contractId}/orders`));
        const expectResult = [getObjectFromModel(createdOrder)];
