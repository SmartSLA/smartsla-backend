...
module.exports = dependencies => {
   ...
   * @return {Promise} Resolve on success with result: { total_count: number, list: [Contract1, Contract2, ...] }
   */
  ...
  function _search(options = {}, callback) {
    ...
    return coreElasticsearch.client((err, elascticsearchClient) => {
      ...
      return elascticsearchClient.search({
        ...
        const contracts = list.map(hit => _.extend(hit._source, { _id: hit._source.id }));
